...
4. Update task status in tasks.md → IN_PROGRESS
```
...
Output: TASKS.md — reviewed by tech lead
```

### Phase 5: Implement (RARV cycle per task)
```
Input: tasks.md, AGENTS.md, CONSTITUTION.md, CONTINUITY.md
For each task:
  1. RARV cycle (above)
  2. Audit trail entry
  3. Task status → DONE
...
Location:   .agents/loki/memory/episodic/
...
Location:   .agents/loki/memory/semantic/PROJECT_KNOWLEDGE_TEMPLATE.md
...
```
Location:   .agents/loki/memory/CONTINUITY.md
...
See `loki/README.md` for Loki Mode setup and persona guide.
