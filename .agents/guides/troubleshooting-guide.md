...
**Symptoms:** Running `python3 init.py` throws an error about missing modules.
...
2. Run standard installation procedures: `pip install -r requirements.txt` (if applicable) or use the interactive `python3 init.py` prompt.
...
**Solution:** Run `python3 init.py` and input `y` when asked to "Overwrite/Update it?".
...
**Solution:** Tell the agent: "Stop. Escalate issue. Read self-heal limit constraints." You can also manually add the failure to `.agents/loki/memory/CONTINUITY.md` so the agent skips it next time.

### Tasks getting lost or corrupted
**Symptoms:** `TASKS.md` is out of sync or tasks disappear.
**Cause:** Concurrent edits by human and agent, or an interrupted session.
**Solution:**
1. Run `gabbe sync` (if using GABBE CLI 2.0).
2. Manually review `.agents/loki/memory/PROJECT_STATE.md` and edit `TASKS.md` to match reality.
...
**Cause:** Multiple terminal windows or an active agent are trying to access `.gabbe/state.db` simultaneously.
**Solution:** Ensure no background processes are running `gabbe` commands. If necessary, delete the `.gabbe/state.db-journal` file.
