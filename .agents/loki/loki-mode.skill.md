...
```
1. Check: does loki/memory/PROJECT_STATE.md exist AND phase != S00_INITIALIZED?
...
1. Create directory structure if missing:
   mkdir -p loki/memory/episodic/SESSION_SNAPSHOT
   mkdir -p loki/memory/semantic
   mkdir -p loki/personas

2. Initialize memory files:
   - loki/memory/PROJECT_STATE.md    → set phase: S01, status: IN_PROGRESS
   - loki/memory/AUDIT_LOG.md        → write header + first entry: LOKI_INIT
   - loki/memory/CONTINUITY.md       → write header (empty failures list)

3. Announce to user:
   "Loki Mode initialized. Starting [PROJECT_NAME] from scratch.
    Current phase: S01_REQUIREMENTS
    All checkpoints will be saved to loki/memory/"
```
...
Checkpoint: sdlc-checkpoint.skill S01
  - Write: loki/memory/episodic/SESSION_SNAPSHOT/S01_requirements.md
  - Update: loki/memory/PROJECT_STATE.md → phase: S01, status: DONE
  - Log:    loki/memory/AUDIT_LOG.md → PHASE_TRANSITION: S01 complete
```
...
Output:  tasks.md with T-NNN IDs, status: TODO, tags: [PARALLEL] if applicable
...
Implementation Loop (repeats until all tasks DONE):
  1. orch-planner analyzes `tasks.md`:
  ...
  4. **Execution (Parallel/Batch)**:
     - For each task in current batch (concurrently if model supports):
       ...
       ACT:     Write failing test (TDD Red — MANDATORY)
                ...
                Update tasks.md: status → IN_PROGRESS
...
At interruption:
```
[LOKI] State saved to: loki/memory/episodic/SESSION_SNAPSHOT/
