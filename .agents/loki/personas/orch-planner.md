...
The execution coordinator of the Engineering Swarm. Reads tasks.md, determines the
...
Load on activation:
  ...
  - loki/memory/PROJECT_STATE.md (current SDLC phase)
  - loki/memory/AUDIT_LOG.md (recent actions)
...
- Updated tasks.md (status tracking: TODO → IN_PROGRESS → DONE)
...
Task ID:     T-NNN
Persona:     eng-[type]
Description: [exact task description from tasks.md]
Files:       [specific files to create/modify]
AC:          [acceptance criteria — verbatim from tasks.md]
...
When a task is BLOCKED (self-heal exhausted):
  ...
  3. Mark task: BLOCKED in tasks.md
...
loki-mode → orch-planner:
  ...
  Input: tasks.md with T-001 to T-067
