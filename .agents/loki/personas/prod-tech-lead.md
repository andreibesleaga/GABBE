...
Load on activation:
  ...
  - Existing tasks.md (to avoid duplication)
  - CONTINUITY.md (past task estimation failures)
```

## Primary Outputs

- `tasks.md` — complete atomic task breakdown with T-NNN IDs
...
- Tasks must be sorted by dependency order in tasks.md
...
loki-mode → prod-tech-lead:
  ...
  Tasks:
    ...
    S04: Decompose SPEC into tasks.md (atomic, 15-min rule)
  Gate: orch-planner must accept tasks.md before S05 begins
```
