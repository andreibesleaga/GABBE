...
#   3. Creates loki/memory/ directory structure
...
# ─────────────────────────────────────────────────────────────────────────────
# Step 3: Create loki/memory/ directory structure
# ─────────────────────────────────────────────────────────────────────────────
echo -e "${YELLOW}Step 3: Initializing loki/memory/ directory structure${NC}"

MEMORY_DIR="${AGENTS_DIR}/loki/memory"
mkdir -p "${MEMORY_DIR}/episodic/SESSION_SNAPSHOT"
mkdir -p "${MEMORY_DIR}/semantic"
echo -e "  ${GREEN}✓${NC} loki/memory/episodic/SESSION_SNAPSHOT/ created"
echo -e "  ${GREEN}✓${NC} loki/memory/semantic/ created"
...
```
EOF
  echo -e "  ${GREEN}✓${NC} loki/memory/PROJECT_STATE.md initialized"
else
  echo -e "  ${YELLOW}!${NC} loki/memory/PROJECT_STATE.md already exists (skipped)"
fi
...
EOF
  echo -e "  ${GREEN}✓${NC} loki/memory/AUDIT_LOG.md initialized"
else
  echo -e "  ${YELLOW}!${NC} loki/memory/AUDIT_LOG.md already exists (skipped)"
fi
...
EOF
  echo -e "  ${GREEN}✓${NC} loki/memory/CONTINUITY.md initialized"
else
  echo -e "  ${YELLOW}!${NC} loki/memory/CONTINUITY.md already exists (skipped)"
fi
...
# Note: we do NOT gitignore loki/memory/ — project memory should be committed
...
echo -e "  .agents/loki/memory/ (initialized)"
echo ""
