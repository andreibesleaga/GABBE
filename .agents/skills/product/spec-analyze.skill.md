---
name: spec-analyze
description: Check consistency and alignment between spec.md, plan.md, and tasks.md — detect drift and mismatches
...
## Goal
Verify that the three core planning artifacts — PRD.md/spec.md, plan.md, and tasks.md — are fully aligned. Detect any requirements that have no implementation plan, any plan items with no tasks, and any tasks with no requirement.
...
1. **Load all three artifacts**
   ...
   - Read tasks.md — get all atomic tasks with their status
...
3. **Check Plan → Task coverage**
   ...
   - Does tasks.md have at least one task for this plan item?
   - Are there plan items with no corresponding tasks?
   - Flag: `[NO_TASKS] Plan item: "Implement email verification" — 0 tasks in tasks.md`
