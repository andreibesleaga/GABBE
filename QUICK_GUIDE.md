...
```
GABBE/
  init.py              <- Installer Wizard (Run this first)
  ...
  .agents/             <- The actual kit payload
      ...
      setup-context.sh <- Internal wiring script (wrapped by init.py)
...
### 3. Scripts (`.agents/setup-context.sh`, `init.py`)
- **What:** Automation tools.
- **Who:** The HUMAN runs them.
- **When:** Once at project start (init.py) or if symlinks break (setup-context.sh).
- **How:** Run from terminal: `python3 init.py`.
...
| **Setup** | Runs `init.py`, feeds `SETUP_MISSION.md`. | Reads mission, analyzes stack, updates `AGENTS.md`. |
...
| **Tasks** | Reviews and **Approves** `TASKS.md`. | **Decomposes** work into 15m tasks. |
...
**Universal Skill Compiler** (what init.py does):
...
**Artifact chain integrity:** spec-analyze.skill checks that spec.md ↔ plan.md ↔ tasks.md are aligned. Run it whenever artifacts change.
...
| **Episodic** | `loki/memory/episodic/` | Past decisions, failed experiments per session | Project |
| **Semantic** | `loki/memory/semantic/` | Crystallized facts (auth quirks, schema conventions) | Project |
...
| File | Purpose |
|---|---|
| `loki/memory/PROJECT_STATE.md` | Current SDLC phase, last checkpoint, blockers |
| `loki/memory/CONTINUITY.md` | **Read every session** — past failures to avoid |
| `loki/memory/AUDIT_LOG.md` | Append-only log of all decisions and outcomes |
| `loki/memory/episodic/SESSION_SNAPSHOT/` | Per-milestone state snapshots |
| `loki/memory/semantic/PROJECT_KNOWLEDGE_TEMPLATE.md` | Crystallized project facts |
...
> spec.md/tasks.md are the control interface, not byproducts. Manage artifacts; code follows.

```
Wrong mental model: "Write code, then document it"
Right mental model:  "Write spec.md -> human approves -> write tasks.md -> code emerges from tasks"

The PRD.md is the truth. The spec.md is the truth. The tasks.md is the control panel.
Code is just the output of working through tasks.md correctly.
```
...
Actions:
  - Run `init.py` to create tool-specific symlinks (commands, architecture rules, forbidden actions)
...
Actions:
  ...
  - Set up loki/memory/ for long-running projects
