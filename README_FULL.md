...
**Universal Skill Compiler:** The `init.py` script automatically configures skills for your specific tool:
...
Read more in [loki/README.md](.agents/loki/README.md).
...
graph TB
    ...
    subgraph "Memory Layer"
        M1[loki/memory/PROJECT_STATE.md<br/>Current SDLC phase] --> M2[loki/memory/AUDIT_LOG.md<br/>Append-only decision history]
        M2 --> M3[loki/memory/episodic/<br/>Per-session decision logs]
        M3 --> M4[loki/memory/episodic/SESSION_SNAPSHOT/<br/>Per-milestone snapshots]
        M5[loki/memory/CONTINUITY.md<br/>Past failures - checked every session] --> M6[loki/memory/semantic/<br/>Crystallized project knowledge]
    end
    subgraph "Orchestration Layer — Large Projects"
        L[loki/loki-mode.skill.md<br/>Master orchestration] --> P[loki/personas/<br/>30+ role-based agent personas]
        L --> M1
    end
...
flowchart TD
    ...
    L --> M[tasks.md decomposition — 15-min rule]
...
2.  **Format:** Use the standard YAML header + Markdown body:
    ...
    ```
3.  **Register:** Run `init.py` (or `.agents/setup-context.sh`) to symlink changes.
...
See `loki/README.md` for full Loki Mode documentation.
...
- Check `loki/memory/CONTINUITY.md` — past failures should be logged here
...
- All memory is stored in `loki/memory/` — it persists across sessions
