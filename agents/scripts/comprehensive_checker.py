...
def main():
    ...
    existing = {"skills": set(), "templates": set(), "guides": set(), "personas": set()}
    ...
    print(
        f"Discovered {len(existing['skills'])} skills, {len(existing['templates'])} templates, {len(existing['guides'])} guides, {len(existing['personas'])} personas.\n"
    )

    # 2. Search for mentions in root and docs/
    mentioned = {"skills": set(), "templates": set(), "guides": set(), "personas": set()}
