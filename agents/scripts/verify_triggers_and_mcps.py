...
def verify_triggers_and_mcps():
    ...
    print(
        f"{GREEN}✓ Loaded {len(mcp_servers)} MCP Servers from Config: {', '.join(list(mcp_servers)[:5])}...{NC}"
    )
    ...
    prompt_skill_regex = re.compile(
        r"(?:use|run)\s+(?:the\s+)?([a-zA-Z0-9-]+)\s*skill", re.IGNORECASE
    )
    ...
    for doc in docs_to_check:
        ...
        for mention in mentions:
            ...
            if mention in skill_triggers or mention in all_triggers:
                ...
                print(
                    f"{YELLOW}! WARNING: Document '{doc.name}' uses skill/trigger '{mention}' which is not in the index.{NC}"
                )
                warnings += 1
    ...
    if visual_skill_path.exists():
        ...
        for rmcp in required_mcps:
            ...
            if mcp_key:
                print(
                    f"{GREEN}✓ Mapped '{rmcp}' capability in visual skill to MCP Server key: {mcp_key}{NC}"
                )
            else:
                print(
                    f"{RED}x FAIL: Visual skill requires '{rmcp}' but no matching MCP Server found in MCP_CONFIG_TEMPLATE.json!{NC}"
                )
                errors += 1
