...
def simulate_sdlc_flow(ctx, memory_dir, episodic_dir):
    ...
    phases = [
        ...
        (
            "S01",
            "requirements",
            ["templates/product/PRD_TEMPLATE.md", "skills/product/spec-writer.skill.md"],
        ),
        (
            "S02",
            "architecture",
            [
                "templates/architecture/C4_ARCHITECTURE_TEMPLATE.md",
                "skills/architecture/arch-design.skill.md",
            ],
        ),
        ...
        (
            "S05",
            "implementation",
            ["skills/coding/tdd-cycle.skill.md", "skills/core/self-heal.skill.md"],
        ),
        (
            "S06",
            "testing",
            ["templates/coding/TEST_PLAN_TEMPLATE.md", "skills/core/integrity-check.skill.md"],
        ),
        (
            "S07",
            "security",
            ["templates/security/SECURITY_CHECKLIST.md", "skills/security/security-audit.skill.md"],
        ),
...
def verify_edge_cases(ctx):
    ...
    ctx.assert_exists(
        AGENTS_DIR / "skills/core/session-resume.skill.md", "Resuming continuity skill"
    )
    ctx.assert_exists(
        AGENTS_DIR / "templates/core/SESSION_SNAPSHOT_TEMPLATE.md",
        "Session state snapshot template",
    )
    ctx.assert_exists(AGENTS_DIR / "skills/core/sdlc-checkpoint.skill.md", "Checkpointing skill")

    print("\n--- Bug Fixing ---")
    ctx.assert_exists(
        AGENTS_DIR / "templates/core/BUG_REPORT_TEMPLATE.md", "Bug replication template"
    )
    ...
    ctx.assert_exists(
        AGENTS_DIR / "skills/coding/performant-nodejs.skill.md", "Node.js optimization skill"
    )
    ctx.assert_exists(
        AGENTS_DIR / "skills/coding/performant-python.skill.md", "Python optimization skill"
    )
    ...
    ctx.assert_exists(
        AGENTS_DIR / "memory/episodic/DECISION_LOG_TEMPLATE.md", "Episodic decision log template"
    )
    ctx.assert_exists(
        AGENTS_DIR / "memory/semantic/PROJECT_KNOWLEDGE_TEMPLATE.md", "Semantic knowledge template"
    )
    ctx.assert_exists(
        AGENTS_DIR / "memory/semantic/VECTOR_DB_CONFIG_TEMPLATE.json", "Vector DB config template"
    )
    ctx.assert_exists(
        AGENTS_DIR / "templates/core/AUDIT_LOG_TEMPLATE.md", "Append-only project decision log"
    )

    print("\n--- AGENTS.md & CONSTITUTION.md ---")
    # Template must exist for init.py to generate AGENTS.md
    ctx.assert_exists(
        AGENTS_DIR / "templates/coordination/AGENTS_TEMPLATE.md", "AGENTS.md template for init.py"
    )
    ...
    ctx.assert_exists(
        AGENTS_DIR / "templates/architecture/LEGACY_AUDIT_TEMPLATE.md",
        "Legacy tech boundaries audit",
    )
    ctx.assert_exists(
        AGENTS_DIR / "skills/architecture/enterprise-patterns.skill.md",
        "Strangler fig pattern etc.",
    )

    print("\n--- Visual Design & Architecture ---")
    ctx.assert_exists(
        AGENTS_DIR / "skills/architecture/visual-whiteboarding.skill.md",
        "Visual Map Whiteboarding skill",
    )
    ctx.assert_exists(
        AGENTS_DIR / "templates/core/WHITEBOARD_DESIGN_TEMPLATE.md", "Whiteboard mapping template"
    )
    ctx.assert_exists(
        AGENTS_DIR / "guides/ai/visual-mcp-integration.md", "Visual vs Text diagramming guide"
    )
    ctx.assert_exists(
        AGENTS_DIR / "guides/planning/visual-product-specs.md", "Visual Product Design Phase guide"
    )
    ctx.assert_exists(AGENTS_DIR / "skills/product/visual-specs.skill.md", "Visual Specs skill")
    ctx.assert_exists(
        AGENTS_DIR / "templates/product/VISUAL_SPEC_PACKAGE_TEMPLATE.md",
        "Visual Spec Package template",
    )
    ...
    ctx.assert_exists(
        AGENTS_DIR / "skills/coding/sketch-to-diagram.skill.md", "Sketch-to-Diagram skill"
    )
    ctx.assert_exists(
        AGENTS_DIR / "templates/coding/SKETCH_TO_DIAGRAM_TEMPLATE.md",
        "Sketch-to-Diagram recognition template",
    )
    ctx.assert_exists(
        AGENTS_DIR / "guides/ai/agent-only-cognition.md", "Agent-Only Cognition guide"
    )
...
def simulate_cognitive_sdlc_flow(ctx, memory_dir, episodic_dir):
    ...
    for skill in required_cognitive_skills:
        ctx.assert_exists(
            AGENTS_DIR / skill, f"Cognitive skill {skill} loaded for cross-phase operations"
        )
    ...
    for phase in cognitive_phases:
        ...
        (memory_dir / "AUDIT_LOG.md").open("a").write(
            f"- [BRAIN PREDICTION]: Phase {phase} expected behavior.\n"
        )

        # 2. Simulate Error / Self-Healing Trigger
        (memory_dir / "CONTINUITY.md").open("a").write(
            f"Failure observed at {phase}. Triggering self-heal.skill.\n"
        )

        # 3. Simulate Meta-Optimization / Brain Evolution
        (episodic_dir / "meta_optimization.md").open("a").write(
            f"Updated prompts and skills based on {phase} failure.\n"
        )
...
def simulate_unified_architecture_integration(ctx, memory_dir, episodic_dir):
    ...
    (memory_dir / "WORKING_MEMORY.md").write_text(
        "Sensory Input via Excalidraw MCP: Architecture diagram parsed.\n"
    )
    ctx.assert_exists(
        memory_dir / "WORKING_MEMORY.md", "Sensory MCP data successfully captured in Working Memory"
    )

    # 2. Cost-Benefit Router Classification
    (episodic_dir / "routing_log.md").write_text(
        "Router evaluated Working Memory: Requires REMOTE LLM due to visual complexity.\n"
    )
    ctx.assert_exists(
        episodic_dir / "routing_log.md", "Cost-Benefit Router successfully classified the task"
    )

    # 3. Brain Mode Processing
    (memory_dir / "AUDIT_LOG.md").open("a").write(
        "- [ORCHESTRATION]: Brain Mode delegated diagram analysis to visual-specs skill.\n"
    )
    ...
    ctx.assert_exists(
        semantic_dir / "GENETIC_POOL.md",
        "Genetic Evolution engine successfully updated semantic memory",
    )
...
def verify_brain_and_research(ctx):
    ...
    ctx.assert_exists(
        AGENTS_DIR / "skills/brain/active-inference.skill.md", "Active Inference Engine"
    )

    print("\n--- Self-Healing & Auto-Updates ---")
    ctx.assert_exists(
        AGENTS_DIR / "skills/coordination/meta-optimize.skill.md", "Meta-optimization skill"
    )
    ctx.assert_exists(
        AGENTS_DIR / "skills/brain/self-improvement.skill.md", "Self improvement skill"
    )
    ...
    ctx.assert_exists(
        AGENTS_DIR / "skills/brain/cognitive-architectures.skill.md", "Platform Intelligence"
    )
...
def run_internal_validation(ctx):
    ...
    for script in scripts_to_run:
        ...
        if result.returncode == 0:
            print(
                f"{GREEN}✓ PASS: {script_path.name} verified the structural soundness successfully.{NC}"
            )
            ctx.passed += 1
        else:
...
def verify_init_logic(ctx):
    ...
    try:
        ...
        if tech_map:
            print(
                f"{GREEN}✓ PASS: init.py successfully parsed {len(tech_map)} tech categories from dynamic skills.{NC}"
            )
            ctx.passed += 1
        else:
...
def main():
    ...
    if ctx.failed == 0:
        print(
            f"{GREEN}All {ctx.passed} scenario assertions PASSED. The framework elegantly supports the documented flows.{NC}"
        )
        sys.exit(0)
    else:
