...
def tmp_project(tmp_path):
    ...
    with patch("gabbe.config.PROJECT_ROOT", tmp_path), patch(
        "gabbe.config.GABBE_DIR", gabbe_dir
    ), patch("gabbe.config.DB_PATH", db_path), patch("gabbe.config.TASKS_FILE", tasks_file), patch(
        "gabbe.config.REQUIRED_FILES", required_files
    ), patch(
        "gabbe.config.GABBE_POLICY_FILE", policy_file
    ), patch(
        "gabbe.policy.GABBE_POLICY_FILE", policy_file
    ), patch(
        "gabbe.database.GABBE_DIR", gabbe_dir
    ), patch(
        "gabbe.database.DB_PATH", db_path
    ), patch(
        "gabbe.sync.TASKS_FILE", tasks_file
    ), patch(
        "gabbe.verify.PROJECT_ROOT", tmp_path
    ), patch(
        "gabbe.verify.GABBE_DIR", gabbe_dir
    ), patch(
        "gabbe.verify.REQUIRED_FILES", required_files
    ), patch(
        "gabbe.brain.PROJECT_ROOT", tmp_path
    ), patch(
        "gabbe.brain.REQUIRED_FILES", required_files
    ), patch(
        "gabbe.audit.GABBE_DIR", gabbe_dir
    ):
