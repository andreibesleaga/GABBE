...
def main():
    ...
    # GABBE CLI: n
    seq_dynamic_off = ["1", "", "", "1", "1", "3", "", "6", "7", "n", "n", "n", "n", "7"]

    try:
        run_test_scenario(
            "Dynamic Setup ON", seq_dynamic_on, expect_bootstrap=True, expect_setup=False
        )
        # Restore init.py state
        os.chdir(original_cwd)
        run_test_scenario(
            "Dynamic Setup OFF", seq_dynamic_off, expect_bootstrap=False, expect_setup=True
        )
    finally:
        os.chdir(original_cwd)
